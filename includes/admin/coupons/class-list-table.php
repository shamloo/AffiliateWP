...
class AffWP_Coupons_Table extends List_Table {
	...
	public function get_columns() {
		$columns = array(
			...
			'coupon_method' => __( 'Coupon Method', 'affiliate-wp' ),
			'status'        => _x( 'Status', 'coupon', 'affiliate-wp' ),
			'date'          => _x( 'Date', 'coupon', 'affiliate-wp' ),
	 ...
	 * @param \AffWP\Affiliate\Coupon $coupon Current coupon object.
	 */
	public function column_owner( $coupon ) {

		// Handle for back-compat first (no owners pre-1.9.5).
		if ( 0 === $coupon->owner ) {
			$value = _x( '(none)', 'coupon owner', 'affiliate-wp' );
		} else {
			$user = get_user_by( 'id', $coupon->owner );
			// If the owner exists, use it.
			if ( $user ) {
				$value = sprintf( '%1$s %2$s',
					affwp_admin_link( 'coupons', esc_html( $user->data->display_name ), array( 'owner' => $coupon->owner ) ),
					sprintf( _x( '(User ID: %d)', 'coupon owner ID', 'affiliate-wp' ),
						esc_html( $coupon->owner )
					)
				);
			} else {
		...
		/**
		 * Filters the value of the 'Generated By' column in the coupons list table.
		 *
		 * @since 2.1.5
		 *
		 * @param string                  $value  Coupon owner display name.
		 * @param \AffWP\Affiliate\Coupon $coupon Current coupon object.
		 */
		return apply_filters( 'affwp_coupon_table_owner', $value, $coupon );
	}

	/**
	 * Renders the 'Coupon Method' column.
	 *
	 * @access public
	 * @since 2.1
	 *
	 * @param \AffWP\Affiliate\Coupon $coupon Current coupon object.
	 * @return string Coupon method.
	 */
	public function column_coupon_method( $coupon ) {
		$value = empty( $coupon->coupon_method ) ? __( '(none)', 'affiliate-wp' ) : esc_html( $coupon->coupon_method );

		/**
		 * Filters the value of the 'Coupon Method' column in the coupons list table.
		 *
		 * @since 2.1
		 *
		 * @param string                  $value  Coupon method.
		 * @param \AffWP\Affiliate\Coupon $coupon Current coupon object.
		 */
		return apply_filters( 'affwp_coupon_table_coupon_method', $value, $coupon );
	}
	 ...
	 *
	 * @param \AffWP\Affiliate\Coupon $coupon Current coupon object.
	 * @return string Localized coupon date.
	 */
	public function column_date( $coupon ) {
		$value = date_i18n( get_option( 'date_format' ), strtotime( $coupon->date ) );

		/**
		 * Filters the value of the 'Date' column in the coupons list table.
		 *
		 * @since 2.1
		 *
		 * @param string                  $value  Localized coupon date.
		 * @param \AffWP\Affiliate\Coupon $coupon Current coupon object.
		 */
		return apply_filters( 'affwp_coupon_table_date', $value, $coupon );
	}
